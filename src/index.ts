...
import { VISION_SCHEMA } from './schema.ts'
import { readImage } from './zai.ts'

/** The shape of the webServer service we consume (duck-typed, no host import). */
...
const EXT: Record<string, string> = { png: '.png', jpg: '.jpg', gif: '.gif', webp: '.webp' }

/** Write pasted bytes to a private temp file and return its path. */
export function savePaste(bytes: Uint8Array): { path: string } {
  const kind = sniffImageType(bytes)
  if (kind === undefined) throw new Error('dsh-eyes: not a recognized image (png/jpeg/gif/webp)')
...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => renderEvidence(value as never),
    },
  ...
  ctx.inject(['webServer'], (scope) => {
    const server = (scope as unknown as { webServer: WebServerLike }).webServer
    const handler: (req: IncomingMessage, res: ServerResponse) => Promise<void> = async (req, res) => {
      const chunks: Buffer[] = []
      let total = 0
